...
import { userMessage, userMessageCheck, validateText, } from "./contact-us-text.js";
...
contactForm.addEventListener("submit", (event) => {
    ...
    if (validationResult !== "valid") {
        updateValidationStatus(false, "Your name is invalid", userName, userNameCheck);
        return;
    }
    const phoneValue = userPhone.value.replace(/(?!^\+)[^0-9 ]/g, "");
    const countryCode = countrySelect.value + " ";
    if (!validatePhone(phoneValue, countryCode))
        return;
