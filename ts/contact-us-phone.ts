...
export const userPhone = document.getElementById(
  "userPhone"
) as HTMLInputElement;
export const countrySelect = document.getElementById(
  "country-select"
...
export function validatePhone(
  ...
  if (
    phoneWithoutCode.length > maxDigits ||
    phoneWithoutCode.length < minDigits
  ) {
    updateValidationStatus(
      false,
      `The phone number must contain between ${minDigits}  <br>
      and ${maxDigits} characters following the country code.`,
      userPhone,
      userPhoneCheck
    );
    return false;
